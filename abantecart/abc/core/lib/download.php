...
final class ADownload
{
        ...
        if ($ids) {
            ...
            foreach ($result->rows as $row) {
                ...
                if (in_array($attributes[$row['attribute_id']]['element_type'], $attributes_with_options)) {
                    ...
                    if (!H::has_value($row['value'])
                        && in_array($mode, ['to_customer', 'to_display'])) {
                        continue;
                    }
    ...
    public function sendDownload($download_info = [])
    {
        ...
        if (!$download_info || !$this->isFileAvailable($download_info['filename'])) {
            $error_text = 'Unable to download file '.ABC::env('DIR_RESOURCES').$download_info['filename']
        ...
        if ($download_info['expire_date'] != '' && H::dateISO2Int($download_info['expire_date']) < time()) {
            return false;
        }
    ...
    public function isFileAvailable($resource_path = '')
    {
        ...
        if (!is_file(ABC::env('DIR_RESOURCES').$resource_path)
            || !is_readable(ABC::env('DIR_RESOURCES').$resource_path)) {
            return false;
        }
    ...
    public function getTextStatusForOrderDownload($download_info)
    {
        ...
        } elseif (H::dateISO2Int($download_info['expire_date']) < time()) {
        ...
        if (!$this->isFileAvailable($download_info['filename'])) {
            $err = new AError('Error: file "'.$download_info['filename'].'" (download_id = '.$download_info['order_id']
                .') of order #'.$download_info['order_id'].' is unavailable for download!');
            $err->toLog()->toDebug();
