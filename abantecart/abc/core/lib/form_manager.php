...
class AFormManager
{
            ...
            if ($form->action == "delete") {
                if ($this->form_id) {
                    ...
                    $sql [] = "DELETE FROM ".$this->db->table_name("field_values")
                        ...
                        ." WHERE form_id = '".$this->form_id."')";
                    $sql [] = "DELETE FROM ".$this->db->table_name("field_descriptions")
                        ." WHERE field_id IN ( SELECT field_id FROM ".$this->db->table_name("fields")
                        ." WHERE form_id = '".$this->form_id."')";
                    $sql [] = "DELETE FROM ".$this->db->table_name("fields_group_descriptions")
                        ." WHERE group_id IN ( SELECT group_id FROM ".$this->db->table_name("form_groups")
                        ." WHERE form_id = '".$this->form_id."')";
                    $sql [] = "DELETE FROM ".$this->db->table_name("fields_groups")
                        ." WHERE group_id IN ( SELECT group_id FROM ".$this->db->table_name("form_groups")
                        ." WHERE form_id = '".$this->form_id."')";
                    $sql [] =
                        "DELETE FROM ".$this->db->table_name("form_groups")." WHERE form_id  = '".$this->form_id."'";
                    $sql [] = "DELETE FROM ".$this->db->table_name("fields")." WHERE form_id = '".$this->form_id."'";
                    $sql [] =
                        "DELETE FROM ".$this->db->table_name("pages_forms")." WHERE form_id = '".$this->form_id."'";
                    $sql [] =
                        "DELETE FROM ".$this->db->table_name("form_descriptions")." WHERE form_id = '".$this->form_id
                        ."'";
                    $sql [] = "DELETE FROM ".$this->db->table_name("forms")." WHERE form_id = '".$this->form_id."'";
                ...
                // if form exists
                if ($this->form_id) {
                    $error_text = 'Error: cannot insert form (name: "'.$form->form_name
    ...
    private function _processFieldXML($field, $field_group_id = '', $field_group_sort_order = 0)
    {
        ...
        if ($field->action == "delete") {
            ...
            $sql [] = "DELETE FROM ".$this->db->table_name("fields")." WHERE field_id = '".$field_id."'";
            foreach ($sql as $query) {
                $this->db->query($query);
            }
            return null;
        ...
        if (!in_array($field->element_type, $this->field_types)) {
            $error_text = 'Error: cannot insert(update) form field because it element type: "'.$field->element_type
                .'" is unknown.';
            $error = new AError ($error_text);
            $error->toLog()->toDebug();
            $this->errors = 1;
        ...
        if (!$field_id) { // if new field
            ...
            if ($field->field_descriptions->field_description) {
                foreach ($field->field_descriptions->field_description as $field_description) {
                    ...
                    if (!$language_id) {
                        $error_text =
                            'Error: cannot insert field description because it language: "'.$field_description->language
            ...
            if ($field->field_values->field_value) {
                foreach ($field->field_values->field_value as $field_value) {
                    ...
                    if (!$language_id) {
                        $error_text = 'Error: cannot insert field values because it language: "'.$field_value->language
                            .'" is not exists in database.';
                        $error = new AError ($error_text);
                        $error->toLog()->toDebug();
                        $this->errors = 1;
            ...
            if ($field_group_id) {
                ...
                $query =
                    "SELECT field_id FROM ".$this->db->table_name("fields_groups")." WHERE field_id = '".$field_id."'";
                $result = $this->db->query($query);
            ...
            if ($field->field_descriptions->field_description) {
                foreach ($field->field_descriptions->field_description as $field_description) {
                    ...
                    if (!$language_id) {
                        $error_text =
                            'Error: cannot update field description because it language: "'.$field_description->language
                            .'" is not exists in database.';
                        $error = new AError ($error_text);
                        $error->toLog()->toDebug();
                        $this->errors = 1;
            ...
            if ($field->field_values->field_value) {
                ...
                foreach ($field->field_values->field_value as $field_value) {
                    $language_id = $this->_getLanguageIdByName($field_value->language);
                    if (!$language_id) {
                        $error_text = 'Error: cannot update field values because it language: "'.$field_value->language
                            .'" is not exists in database.';
                        $error = new AError ($error_text);
                        $error->toLog()->toDebug();
                        $this->errors = 1;
    ...
    private function _processFieldGroupXML($field_group)
    {
        ...
        if ($field_group->action == 'delete') {
            if ($field_group_id) {
                ...
                $sql [] = "DELETE FROM ".$this->db->table_name("fields")." WHERE field_id IN ( SELECT field_id FROM "
                    .$this->db->table_name("fields_groups")." WHERE group_id = '".$field_group_id."')";
                $sql [] =
                    "DELETE FROM ".$this->db->table_name("fields_groups")." WHERE group_id = '".$field_group_id."'";
                $sql [] = "DELETE FROM ".$this->db->table_name("form_groups")." WHERE form_id  = '".$this->form_id."'";
                foreach ($sql as $query) {
                    $this->db->query($query);
                }
            } else {
        ...
        if ($field_group->field_group_descriptions->field_group_description) {
            ...
            foreach ($field_group->field_group_descriptions->field_group_description as $field_group_description) {
                ...
                if (!$language_id) {
                    $error_text = 'Error: cannot update field group description because it language: "'
                        .$field_group_description->language.'" is not exists in database.';
                    $error = new AError ($error_text);
                    $error->toLog()->toDebug();
                    $this->errors = 1;
                    continue;
