...
class ALayoutManager
{
    ...
    public function deletePageLayoutByID( $page_id, $layout_id )
    {
        ...
        $this->db->query( "DELETE FROM ".$this->db->table_name( "pages_layouts" )." WHERE layout_id = '".(int)$layout_id."' AND page_id = '".(int)$page_id."'" );
    ...
    protected function _processPage( $layout_id, $page )
    {
        ...
        if ( $page_id ) {
            ...
            if ( ! ( int )$result->row ['layout_id'] ) {
                $sql = "INSERT INTO ".$this->db->table_name( "pages_layouts" )." (layout_id,page_id) VALUES ('".( int )$layout_id."','".( int )$page_id."')";
                $this->db->query( $sql );
            }
        } else { // if page new
    ...
    protected function _processCustomBlock( $layout_id, $block, $parent_instance_id = 0 )
    {
        ...
        $custom_block_info = $this->getBlocksList(
                                    array( 'subsql_filter' => "bd.name = '".(string)$block->custom_block_txt_id
