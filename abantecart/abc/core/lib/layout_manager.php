...
class ALayoutManager
{
    ...
    protected function _getLayoutBlocks( $layout_id = 0 )
    {
        ...
        $sql = "SELECT bl.instance_id AS instance_id,
                ...
                LEFT JOIN ".$this->db->table( "block_layouts" )." AS bl 
    ...
    public function savePageLayout( $data )
    {
        ...
        if ( $layout ['layout_type'] == 0 && ( $page ['controller'] != 'generic' || $data['controller'] ) ) {
            ...
            $this->db->query( "INSERT INTO ".$this->db->table( "pages_layouts" )." 
                                (layout_id,page_id)
    ...
    {
        if ( ! $instance_id ) {
            $this->db->query( "INSERT INTO ".$this->db->table( "block_layouts" )."\n 
    ...
    {
        if ( ! $page_id ) {
            $this->db->query( "INSERT INTO ".$this->db->table( "pages" )." 
                                (   parent_page_id,
                                    controller,
                                    key_param,
                                    key_value,
            ...
            $this->db->query( "DELETE FROM ".$this->db->table( "page_descriptions" )." 
                                WHERE page_id = '".( int )$page_id."'" );
    ...
    public function saveBlock( $data, $block_id = 0 )
    {
        ...
        if ( ! $block_id ) {
            $this->db->query( "INSERT INTO ".$this->db->table( "blocks" )." 
                                (block_txt_id,
                                 controller,
                                 date_added,
                                 date_modified)
    ...
    public function editBlockStatus( $status, $block_id = 0, $custom_block_id = 0, $layout_id = 0 )
    {
        ...
        if ( $block_id && ! $custom_block_id ) {
            ...
            $sql = "SELECT * 
                    FROM ".$this->db->table( 'custom_blocks' )." 
                    WHERE block_id = ".$block_id;
            $result = $this->db->query( $sql );
            if ( $result->num_rows ) {
    ...
    public function saveBlockDescription( $block_id = 0, $custom_block_id = 0, $description = array() )
    {
        ...
        if ( $custom_block_id ) {
            ...
            $this->db->query( "INSERT INTO ".$this->db->table( "custom_blocks" )." (block_id, date_added) 
                                VALUES ( '".$block_id."', NOW())" );
    ...
    protected function _processCustomBlock( $layout_id, $block, $parent_instance_id = 0 )
    {
        ...
        if ( $action == 'delete' ) {
            ...
            if ( isset( $this->custom_blocks[(string)$block->custom_block_txt_id] ) ) {
                ...
                if ( ! $custom_block_id ) {
                    // if block is new
                    $sql = "INSERT INTO ".$this->db->table( "custom_blocks" )." 
                            (block_id, date_added)
                            VALUES ('".$block_id."', NOW())";
                    $this->db->query( $sql );
                    $custom_block_id = $this->db->getLastId();
