...
class APackageManager
{
    ...
    public function unpack($archive_filename, $dest_dir)
    {
        ...
        $this->removeDir($dest_dir.pathinfo(pathinfo($archive_filename, PATHINFO_FILENAME), PATHINFO_FILENAME));

        $unpack_result = AHelperUtils::extractArchive($archive_filename, $dest_dir);
        if($unpack_result){
            $this->chmod_R($dest_dir.$this->session->data['package_info']['tmp_dir'], 0777, 0777);
        }
        return $unpack_result ? true : false;
    }
    ...
    public function getTempDir()
    {
        ...
        if (AHelperUtils::is_writable_dir($tmp_dir) && AHelperUtils::is_writable_dir($tmp_install_dir)) {
            ...
            if ( ! is_writable($dir)) {
                $error_text = 'Error: php tried to use directory '.ABC::env('DIR_SYSTEM')."temp/install".' but it is non-writable. Temporary php-directory '.$dir.' is non-writable too! Please change permissions one of them.'."\n";
