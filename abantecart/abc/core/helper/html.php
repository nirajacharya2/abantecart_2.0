...
class AHelperHtml extends AHelper{
	...
	static function renderStoreMenu($menu, $level = 0){
		...
		foreach ($menu as $item) {
			...
			$result .= '<a' . $class . $href . '>';
			$result .= $item['icon'] ? '<img src="' . ABC::env('HTTPS_DIR_RESOURCES') . $item['icon'] . '" alt="" />' : '';
			$result .= '<span>' . $item['text'] . '</span></a>';

			if (!empty($item['children'])) {
				$result .= "\r\n" . self::renderStoreMenu($item['children'], $level + 1);
			}
			$result .= "</li>\r\n";
		}
		if ($level) {
...
//New menu tree builder (1.2+)
	static  function buildStoreFrontMenuTree($menu_array, $level = 0){
		...
		foreach ($menu_array as $item) {
			...
			//build appropriate menu id and classes for css control
			...
			if ($rl_id) {
				...
				if ($resource['resource_path'] && is_file(ABC::env('DIR_RESOURCES') . 'images/' . $resource['resource_path'])) {
					$result .= '<img class="menu_image" src="' . ABC::env('HTTPS_DIR_RESOURCES') . 'images/' . $resource['resource_path'] . '" alt="" />';
			...
			if (!empty($item['children'])) {
				$result .= "\r\n" . self::buildStoreFrontMenuTree($item['children'], $level + 1);
			}
			$result .= "</li>\r\n";
		}
		if ($level > 0) {
	...
	static  function renderAdminMenu($menu, $level = 0, $current_rt = ''){
		$result = '';
		if ($level) {
		...
		foreach ($menu as $item) {
			...
			if (H::is_html($item['icon'])) {
				...
				if ($item['icon']) {
					$result .= '<img class="menu_image" src="' . ABC::env('HTTPS_DIR_RESOURCES') . $item['icon'] . '" alt="" />';
				} else {
