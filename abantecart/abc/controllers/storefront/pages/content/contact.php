        ...
        if ($this->request->is_POST() && $this->_validate()) {
            ...
            foreach ($form_fields as $field_name => $field_info) {
                if (H::has_value($post_data[$field_name])
                    ...
                    $this->data['mail_template_data']['form_fields'][rtrim($field_details['name'], ':')] =
                        $post_data[$field_name];
                }
            }
            ...
            if ($file_paths) {
                ...
                foreach ($file_paths as $file_info) {
                    ...
                    $this->data['mail_plain_text'] .= "\t".$file_info['display_name'].': '.$basename." ("
                        .round(filesize($file_info['path']) / 1024, 2)."Kb)\r\n";
            ...
            if ($this->config->get('config_duplicate_contact_us_to_message')) {
                $this->messages->saveNotice(
                    sprintf(
                        $this->language->get('entry_duplicate_message_subject'),
            ...
            if (is_file(ABC::env('DIR_RESOURCES').$config_mail_logo)) {
                $mail->addAttachment(
                    ABC::env('DIR_RESOURCES').$config_mail_logo,
        ...
        if ($this->form->getFields()) {
            if($this->customer->isLogged()) {
                ...
                if ($email && !$email['value']){
                    $this->form->assign('email', $this->customer->getEmail());
                }
                $firstName = $this->form->getField('first_name');
                if ($firstName && !$firstName['value']){
                    $this->form->assign('first_name', $this->customer->getFirstName());
                }
            }
