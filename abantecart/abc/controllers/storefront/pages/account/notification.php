...
class ControllerPagesAccountNotification extends AController
{
    ...
    public function main()
    {
        ...
        foreach ($im_drivers as $name => $driver) {
            $this->data['protocols'][$name] = ['name' => $name];
            if (is_object($driver)) {
        ...
        $all_im_settings = $this->customer->getCustomerNotificationSettings();
        ...
        foreach ($sendpoints as $sendpoint => $sendpoint_data) {
            ...
            $point = [];
            $point['title'] = $this->language->get('im_sendpoint_name_'.H::preformatTextID($sendpoint));
            $point['note'] = $this->language->get('im_sendpoint_name_'.H::preformatTextID($sendpoint).'_note');
            $point['warn'] = '';
