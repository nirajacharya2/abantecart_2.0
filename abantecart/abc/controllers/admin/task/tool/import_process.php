...
class ControllerTaskToolImportProcess extends AController
{
    ...
    public function processRows()
    {
        ...
        $this->failed_count = 0;
        $result = $this->process($task_id, $step_id);
    ...
    private function process($task_id, $step_id)
    {
        ...
        if ($file_format == 'internal') {
            ...
            for ($i = $start; $i <= $stop; $i++) {
                $csv_array =
                    $a_data->CSV2ArrayFromFile($filename, array_search($delimiter, $a_data->csvDelimiters), $i, 1);
                if ($csv_array) {
