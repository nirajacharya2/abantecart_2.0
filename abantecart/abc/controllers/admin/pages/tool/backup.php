    ...
    public function main()
    {
        ...
        if ($this->request->is_POST() && $this->validate()) {
            ...
            if (is_uploaded_file($this->request->files['restore']['tmp_name'])) {
                ...
                $job_info = $this->model_tool_backup->createBackupJob('scheduled_backup_'.time(), $this->request->post);
                ...
                if (!$job_info['job_id']) {
                    ...
                    $this->session->data['success'] = sprintf(
                        ...
                        $this->html->getSecureURL('tool/jobs')
                    );
        ...
        $this->data['entry_compress_backup'] = sprintf(
            ...
            str_replace(ABC::env('DIR_ROOT'), '', ABC::env('DIR_BACKUP')),
            str_replace(ABC::env('DIR_ROOT').'/', '', ABC::env('DIR_BACKUP'))
        );
