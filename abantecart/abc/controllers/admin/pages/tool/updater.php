        ...
        if ($this->data['mp_connected']) {
            ...
            if (!empty($updates) && is_array($updates)) {
                foreach ($updates as $key => $version_info) {
                    ...
                    if (H::versionCompare($current_version, $new_version, '>=')) {
                        continue;
                    }
                    ...
                    $this->data['extensions'][$key]['status'] = $this->html->buildElement(
                        [
                            ...
                            'attr'  => 'readonly="true" data-edit-url="'
                                .$this->html->getSecureURL('extension/extensions/edit', '&extension='.$key).'"',
                        ]);

                    if ($version_info['installation_key']) {
                        $this->data['extensions'][$key]['install_url'] = $this->html->getSecureURL(
                            'tool/package_installer',
                            '&extension_key='.$version_info['installation_key']
                        );
                    } else {
