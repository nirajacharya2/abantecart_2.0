...
class ControllerPagesToolGlobalSearch extends AController
{
    ...
    public function main()
    {
        ...
        $this->request->post['search'] =
            $this->request->post['search'] ? $this->request->post['search'] : $this->request->get['search'];
        $this->data['heading_title'] = $this->language->get('heading_title').':&nbsp;&nbsp;&nbsp;&nbsp;'
            .htmlentities($this->request->post ['search'], ENT_QUOTES, ABC::env('APP_CHARSET'));
        ...
        $this->data['grid_inits'] = []; // list of js-functions names for initialization all jqgrids
        if ($this->_validate()) {
            $search_categories_icons =
                $this->model_tool_global_search->getSearchSources($this->request->post['search']);
            ...
            if ($search_categories) {
                ...
                foreach ($search_categories as $search_category) {
                    ...
                    $total =
                        $this->model_tool_global_search->getTotal($search_category, $this->request->post['search']);
                    if ($total < 1) {
                        continue;
                    } else {
                    ...
                    $grid_settings = [
                        ...
                        'url'            => $this->html->getSecureURL('listing_grid/global_search_result',
                            "&search_category=".$search_category."&keyword=".$this->request->post ['search']),
                        'editurl'        => null,
