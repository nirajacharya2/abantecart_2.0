...
class ControllerPagesCatalogProductPromotions extends AController{
	...
	public function main(){
		...
		if ($this->request->is_POST() && $this->_validateForm()) {
			if ($this->request->post['promotion_type'] == 'discount') {
				if (AHelperUtils::has_value($this->request->get['product_discount_id'])) { //update
					$this->model_catalog_product->updateProductDiscount($this->request->get['product_discount_id'],
							$this->request->post);
				} else { //insert
					$this->model_catalog_product->addProductDiscount($this->request->get['product_id'],
							$this->request->post);
				}
			} elseif ($this->request->post['promotion_type'] == 'special') {
				if (AHelperUtils::has_value($this->request->get['product_special_id'])) { //update
					$this->model_catalog_product->updateProductSpecial($this->request->get['product_special_id'],
							$this->request->post);
				} else { //insert
					$this->model_catalog_product->addProductSpecial($this->request->get['product_id'],
							$this->request->post);
				}
			}
			$this->session->data['success'] = $this->language->get('text_success');
			abc_redirect($this->html->getSecureURL('catalog/product_promotions',
		...
		$this->data['delete_discount'] = $this->html->getSecureURL('catalog/product_promotions/delete',
				'&product_id=' . $this->request->get['product_id'] . '&product_discount_id=%ID%');
		$this->data['update_discount'] = $this->html->getSecureURL('catalog/product_discount_form/update',
				'&product_id=' . $this->request->get['product_id'] . '&product_discount_id=%ID%');

		$this->data['product_specials'] = $this->model_catalog_product->getProductSpecials($this->request->get['product_id']);
		$this->data['delete_special'] = $this->html->getSecureURL('catalog/product_promotions/delete',
				'&product_id=' . $this->request->get['product_id'] . '&product_special_id=%ID%');
		$this->data['update_special'] = $this->html->getSecureURL('catalog/product_special_form/update',
				'&product_id=' . $this->request->get['product_id'] . '&product_special_id=%ID%');
		...
		$this->data['button_add_discount'] = $this->html->buildElement(
				array (
						...
						'href'  => $this->html->getSecureURL('catalog/product_discount_form/insert',
								'&product_id=' . $this->request->get['product_id']),
						'style' => 'button1'
				));
		$this->data['button_add_special'] = $this->html->buildElement(
				array (
						...
						'href'  => $this->html->getSecureURL('catalog/product_special_form/insert',
								'&product_id=' . $this->request->get['product_id']),
						'style' => 'button1'
				));
	...
	public function delete(){
		...
		$this->loadLanguage('catalog/product');
		$this->loadModel('catalog/product');
		if (AHelperUtils::has_value($this->request->get['product_discount_id'])) {
			$this->model_catalog_product->deleteProductDiscount($this->request->get['product_discount_id']);
		} elseif (AHelperUtils::has_value($this->request->get['product_special_id'])) {
	...
	private function _validateForm(){
		...
		if (AHelperUtils::has_value($this->request->post['promotion_type'])) {
			if ($this->request->post['date_start'] != '0000-00-00' && $this->request->post['date_end'] != '0000-00-00'
					...
					&& AHelperUtils::dateFromFormat($this->request->post['date_start'],
							$this->language->get('date_format_short')) > AHelperUtils::dateFromFormat($this->request->post['date_end'],
