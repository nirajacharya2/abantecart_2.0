...
class ControllerPagesCatalogAttribute extends AController
{
    ...
    public function update()
    {
        ...
        if ($this->request->is_POST() && $this->validateAttributeForm()) {
            ...
            $this->session->data['success'] = $this->language->get('text_success');
            abc_redirect($this->html->getSecureURL('catalog/attribute/update',
    ...
    protected function getForm()
    {
        ...
        if ($attribute_id && $this->request->is_GET()) {
            ...
            if (H::has_value($attribute_info['settings'])) {
                $attribute_info['settings'] = unserialize($attribute_info['settings']);
            }
        }

        if (H::has_value($this->request->get['attribute_type_id'])) {
            $attribute_type_info =
                $this->attribute_manager->getAttributeTypeInfoById((int)$this->request->get['attribute_type_id']);
        }
        ...
        if (!$attribute_id) {
            ...
            $this->data['action'] = $this->html->getSecureURL('catalog/attribute/update',
    ...
    public function validateAttributeForm()
    {
        ...
        if (!H::has_value($this->request->get_or_post('attribute_type_id'))) {
        ...
        if (H::has_value($this->request->post['regexp_pattern'])) {
            $this->request->post['regexp_pattern'] = trim($this->request->post['regexp_pattern']);
        }
        if (H::has_value($this->request->post['placeholder'])) {
    ...
    {
        $method = (H::has_value($this->request->get['attribute_id']) ? 'update' : 'insert');
        ...
        foreach ($this->data['attribute_types'] as $type_id => $type) {
            ...
            $this->data['tabs'][$type_id] = [
                'text' => $type['type_name'],
                'href' => $method == 'insert' ? $this->html->getSecureURL('catalog/attribute/'.$method,
