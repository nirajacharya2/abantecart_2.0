...
class ControllerPagesExtensionExtensionsStore extends AController
{
    ...
    public function main()
    {
        ...
        $this->document->addStyle(
            [
                'href' => ABC::env('RDIR_ASSETS') . 'css/marketplace.css',
                'rel' => 'stylesheet'
            ]);
        ...
        if (!H::has_value($request_data['sidx'])) {
            $request_data['sidx'] = 'date_modified';
        }
        if (!H::has_value($request_data['sord'])) {
            $request_data['sord'] = 'desc';
        }
        if (H::has_value($request_data['limit'])) {
            $request_data['rows'] = $request_data['limit'];
        }
        $token_param = "";
        if (H::has_value($mp_token)) {
        ...
        $remote_store_product_url = $this->model_tool_mp_api->getMPURL()
            ...
            . '&mp_hash=' . $this->session->data['mp_hash']
        ...
        if ($result['products']['rows']) {
            ...
            if (H::has_value($request_data['keyword'])) {
                $uri .= '&keyword=' . $request_data['keyword'];
            }
            if (H::has_value($request_data['category_id'])) {
                $uri .= '&category_id=' . $request_data['category_id'];
            }
            if (H::has_value($request_data['manufacturer_id'])) {
                $uri .= '&manufacturer_id=' . $request_data['manufacturer_id'];
            }
            if (H::has_value($request_data['purchased_only'])) {
        ...
        $this->data['my_account'] = $this->model_tool_mp_api->getMPURL()
            . '?rt=account/account&mp_token=' . $this->session->data['mp_token']
            . '&mp_hash=' . $this->session->data['mp_hash'];
