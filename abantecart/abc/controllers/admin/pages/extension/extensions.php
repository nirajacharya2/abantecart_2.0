...
class ControllerPagesExtensionExtensions extends AController
{
    ...
    public function main()
    {
        ...
        if (H::has_value($this->request->get_or_post('store_id'))) {
    ...
    public function edit()
    {
        ...
        foreach ($settings as $item) {
            ...
            if (H::has_value((string)$data['template'])) {
        ...
        if (!$this->extension_manager->validateDependencies($extension, H::getExtensionConfigXml($extension))) {
        ...
        if ($ext->getConfig('additional_settings')) {
            ...
            if ($store_id) {
                ...
                $btn_param['link'] = $store_info['config_url']
                                    .'?s='.ABC::env('ADMIN_SECRET').'&rt='.$ext->getConfig('additional_settings');
    ...
    public function install()
    {
        ...
        if (!$this->user->canModify('extension/extensions')) {
            ...
            if ($config === false) {
                $filename = ABC::env('DIR_APP_EXTENSIONS').$this->request->get['extension'].'/config.xml';
            ...
            abc_redirect($this->html->getSecureURL('extension/extensions/edit',
                '&extension='.$this->request->get['extension']));
        }
    }
    ...
    public function uninstall()
    {
        ...
        if (!$this->user->canModify('extension/extensions')) {
            ...
            $result = $this->extension_manager->uninstall($this->request->get['extension'], $ext->getConfig());
            if (!$result) {
                $this->session->data['error'] = implode("<br>", $this->extension_manager->errors);
            }
            abc_redirect($this->html->getSecureURL('extension/extensions/'.$this->session->data['extension_filter']));
