...
{
    public $error = [];
    private $fields = ['name', 'code', 'locale', 'image', 'directory', 'sort_order', 'status'];
    ...
    private function _getForm()
    {
        ...
        if (!isset($this->request->get['language_id'])) {
            ...
            $this->data['update'] = $this->html->getSecureURL(
                ...
                '&id=' . $this->request->get['language_id']);
            $form = new AForm('HS');
        }
            ...
            foreach ($incomplete as $incm_task) {
                ...
                if ($this->user->getUserGroupId() != 1) {
                    ...
                    if ($incm_task['name'] == $task_name) {
                        $tm->updateTask($incm_task['task_id'], ['name' => $task_name . '_' . date('YmdHis')]);
                    }
                }
