...
class ControllerResponsesProductProduct extends AController
{
    ...
    public function category()
    {
        ...
        foreach ($results as $result) {
            ...
            if (!empty($this->request->get['currency']) && !empty($this->request->get['value'])) {
                $price = $this->currency->format((float)$price, $this->request->get['currency'],
    ...
    public function update_option()
    {
        ...
        $option_info = $this->model_catalog_product->getProductOption($this->request->get['product_id'],
    ...
    public function load_option()
    {
        ...
        if ($this->data['option_data']) {
            ...
            $this->data['selectable'] =
                in_array($this->data['option_data']['element_type'], $this->data['elements_with_options']) ? 1 : 0;
            ...
            if ($this->data['option_data']['element_type'] == 'U') {
                ...
                $this->data['entry_upload_dir'] =
                    sprintf($this->language->get('entry_upload_dir'), 'admin/system/uploads/');
            } else {
            ...
            $this->data['update_option_values'] = $this->html->getSecureURL('product/product/update_option_values',
                '&product_id='.$product_id.'&option_id='.$option_id);
    ...
    public function del_option()
    {
        ...
        $this->loadLanguage('catalog/product');
        $this->loadModel('catalog/product');
        $this->model_catalog_product->deleteProductOption($this->request->get['product_id'],
            $this->request->get['option_id']);
        //update controller data
        $this->extensions->hk_UpdateData($this, __FUNCTION__);
    ...
    private function _option_value_form($form)
    {
        ...
        if ($this->data['option_attribute']['attribute_id']) {
            $group_attribute = $this->attribute_manager->getAttributes(array(), $this->data['language_id'],
        ...
        if (isset($this->request->get['product_option_value_id'])) {
            ...
            $item_info = $this->model_catalog_product->getProductOptionValue($this->request->get['product_id'],
        ...
        if (isset($this->data['option_attribute']['group'])) {
            ...
            foreach ($this->data['option_attribute']['group'] as $attribute_id => $data) {
                $this->data['form']['fields']['option_value'] .=
                    '<span style="white-space: nowrap;">'.$data['name'].''.$form->getFieldHtml(array(
                        'type'    => $data['type'],
            ...
            if (in_array($this->data['option_attribute']['element_type'], $this->data['elements_with_options'])) {
                ...
                if ($this->data['option_attribute']['element_type'] == 'U') {
                    ...
                    $arr = array(
                        'type'  => in_array($this->data['option_data']['element_type'],
                            array('T', 'B')) ? 'textarea' : 'input',
    ...
    public function getProductOptionValues($attribute_id, $language_id = 0)
    {
        ...
        $query = $this->db->query("SELECT pov.*, povd.name as value
									...
									LEFT JOIN `".$this->db->table_name('product_option_value_descriptions')."` povd
										ON ( pov.product_option_value_id = povd.product_option_value_id AND povd.language_id = '"
            .(int)$language_id."' )
    ...
    public function buildDownloadForm()
    {
        ...
        if ($download_id) {
            ...
            $form = new AForm('HT');
            $this->data['action'] =
                $this->html->getSecureURL('r/product/product/processDownloadForm', '&product_id='.$product_id);
        }
        $form->setForm(array(
        ...
        $this->data['form_title'] =
            $download_id ? $this->language->get('text_edit_product_file') : $this->language->get('text_create_download');
        if ($product_id) {
            $this->data['file_list_url'] =
                $this->html->getSecureURL('catalog/product_files', '&product_id='.$product_id);
        } else {
    ...
    private function _buildGeneralSubform($form, $download_id, $product_id)
    {
        ...
        $this->data['date_added'] = AHelperUtils::dateISO2Display($file_data['date_added'],
            $this->language->get('date_format_short').' '.$this->language->get('time_format'));
        $this->data['date_modified'] = AHelperUtils::dateISO2Display($file_data['date_modified'],
            $this->language->get('date_format_short').' '.$this->language->get('time_format'));
        ...
        if ($resource_id) {
            $this->data['preview']['href'] =
                $this->html->getSecureURL('common/resource_library/get_resource_preview', '&resource_id='.$resource_id,
                    true);
            ...
            foreach ($file_data['map_list'] as $map_id => $map_name) {
                ...
                $this->data['map_list'][] = array(
                    'href' => $this->html->getSecureURL('catalog/product_files',
                        '&product_id='.$map_id.'&download_id='.$this->data['download_id'], true),
                    'text' => $map_name,
                );
            }
        ...
        $this->data['delete_unmap_href'] = $this->html->getSecureURL('catalog/product_files',
    ...
    private function _buildAttributesSubform($form)
    {
        ...
        if (!$attributes) {
            ...
            foreach ($attributes as $attribute) {
                ...
                $this->data['form']['fields']['attributes'][$attribute['attribute_id']] =
                    $form->getFieldHtml($option_data);
            }
        }
    ...
    public function orderProductForm()
    {
        ...
        if ($order_product_id) {
            ...
            $preset_values['price'] =
                $this->currency->format($order_product_info[0]['price'], $order_info['currency'], $order_info['value'],
                    false);
            $preset_values['total'] =
                $this->currency->format(($order_product_info[0]['price'] * $order_product_info[0]['quantity']),
            ...
            $form_action = $this->html->getSecureURL('sale/order/update',
            ...
            $preset_values['price'] =
                $this->currency->format($product_info['price'], $order_info['currency'], $order_info['value'], false);
            $preset_values['total'] =
                $this->currency->format(($product_info['price'] * $preset_values['quantity']), $order_info['currency'],
                    $order_info['value'], false);

            $form_action =
                $this->html->getSecureURL('sale/order/update', '&order_id='.$order_id.'&product_id='.$product_id);
        }
        ...
        foreach ($product_options as $option) {
            ...
            foreach ($option['option_value'] as $option_value) {
                ...
                && !$order_product_id ? $option_value['product_option_value_id'] : $default_value;
                //early saved value
            ...
            if (count($values)) {
                ...
                $option_data = array(
                    ...
                    'name'           => !in_array($option['element_type'],
                        HtmlElementFactory::getMultivalueElements()) ? 'product[0][option]['
                        .$option['product_option_id'].']' : 'product[0][option]['.$option['product_option_id'].'][]',
                    'value'          => $value,
                    'options'        => $values,
        ...
        if (!$options && $product_info['subtract']) {
            if ($product_info['quantity']) {
                $this->data['column_quantity'] =
                    $this->language->get('column_quantity').' ('.$this->language->get('text_product_in_stock').': '
