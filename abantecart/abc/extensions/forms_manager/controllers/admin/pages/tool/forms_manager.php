...
class ControllerPagesToolFormsManager extends AController
{
    ...
    protected function getForm()
    {
        ...
        if (!$this->config->get('forms_manager_default_sender_name')
            ...
            $this->data['error_warning'] =
                $this->html->convertLinks($this->language->get('forms_manager_error_empty_sender'));
        }
        ...
        $this->data['form']['form_open'] = $form->getFieldHtml([
            'type'   => 'form',
            'name'   => 'new_fieldFrm',
            'action' => $this->html->getSecureURL('forms_manager/fields/addField',
                '&form_id='.$this->data['form_data']['form_id']),
        ]);
        ...
        $this->data['urls'] = [];
        $this->data['urls']['get_fields_list'] = $this->html->getSecureURL('forms_manager/fields/get_fields_list',
            '&form_id='.$this->request->get['form_id']);
        $this->data['urls']['load_field'] = $this->html->getSecureURL(
            'forms_manager/fields/load_field',
            '&form_id='.$this->request->get['form_id']
        );
        $this->data['urls']['update_field'] = $this->html->getSecureURL(
            'forms_manager/fields/updateField',
            '&form_id='.$this->request->get['form_id']
        );
        $this->data['urls']['update_form'] = $this->html->getSecureURL(
    ...
    public function edit_block()
    {
        ...
        if ($this->request->is_POST() && $this->validateBlockForm($this->request->post)) {
            ...
            $this->session->data ['success'] = $this->language->get('text_success');
            abc_redirect($this->html->getSecureURL('tool/forms_manager/edit_block',
    ...
    protected function getBlockForm()
    {
        ...
        $result = $this->model_tool_forms_manager->getForms(['filter' => ['status' => 1]]);
        $forms = [];
