...
class ModelToolFormsManager extends Model
{
    ...
    public function addForm( $data )
    {
        ...
        if ( ! $this->getFormIdByName( $data['form_name'] ) ) {
            ...
            $q = 'INSERT INTO '.$this->db->table_name( "forms" ).'
                    SET
    ...
    {
        $this->db->query(
            'DELETE FROM '.$this->db->table_name( "forms" ).'
    ...
    {
        $this->db->query(
            'DELETE FROM '.$this->db->table_name( "form_descriptions" ).'
            WHERE form_id = "'.(int)$form_id.'"'
        );
    }
    ...
    public function updateForm( $data )
    {
        ...
        if ( $this->getFormNameById( $data['form_id'] ) !== false ) {
            ...
            if ( ! empty( $cols ) ) {
                $this->db->query(
                    'UPDATE '.$this->db->table_name( "forms" ).'
                    SET '.implode( ',', $cols ).'
    ...
    public function updateFormFieldData( $data )
    {
        ...
        if ( $update ) {
            ...
            $sql = "UPDATE ".$this->db->table_name( 'fields' )."
                    SET ".implode( ",\n ", $update )."
    ...
    {
        $this->db->query(
            'DELETE FROM '.$this->db->table_name( "field_values" ).'
    ...
    public function getFormIdByName( $name )
    {
        ...
        $results = $this->db->query(
            'SELECT form_id FROM '.$this->db->table_name( "forms" ).'
    ...
    {
        $results = $this->db->query(
            'SELECT form_name FROM '.$this->db->table_name( "forms" ).'
    ...
    public function addField( $form_id, $data )
    {
        ...
        if ( $this->isFieldNameUnique( $form_id, $data['field_name'] ) ) {
            ...
            $sql = "INSERT INTO ".$this->db->table_name( 'fields' )."
                    SET
                        form_id = ".(int)$form_id.",
    ...
    public function isFieldNameUnique( $form_id, $field_name, $field_id = 0 )
    {
        ...
        $result = $this->db->query(
            "SELECT field_id FROM ".$this->db->table_name( "fields" )."
            WHERE form_id = '".(int)$form_id."'
    ...
    public function getField( $field_id )
    {
        ...
        $query = $this->db->query( "
            ...
            FROM ".$this->db->table_name( "fields" )." f
            LEFT JOIN ".$this->db->table_name( "field_descriptions" )." fd 
                ON ( f.field_id = fd.field_id AND fd.language_id = '".(int)$this->session->data['content_language_id']."' )
    ...
    {
        $this->db->query(
            'DELETE FROM '.$this->db->table_name( "fields" ).'
            WHERE form_id = "'.(int)$form_id.'"
