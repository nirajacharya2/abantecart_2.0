...
<div id="content" class="panel panel-default">
    <div class="panel-heading col-xs-12">
        <div class="primary_content_actions pull-left">
            <div class="btn-group mr10 toolbar">
                ...
                echo $this->getHookVar( 'extension_toolbar_buttons' ); ?>
            </div>
        </div>
    ...
    <div class="panel-body panel-body-nopadding">
        <div class="row">
            <div class="col-sm-3 mt10"><img src="<?php echo ABC::env('HTTPS_EXT').'default_pp_pro/templates/default/admin/assets/images/all_in_one_solution_logo_u2645_normal.gif'; ?>"/></div>
...
<div class="panel-body panel-body-nopadding tab-content <?php if ( $this->config->get( 'default_pp_pro_test' ) ) {
    echo 'status_test';
} ?>">
    ...
    <?php foreach ( $settings as $name => $field ) {
        ...
        if ( is_int( stripos( $field['value']->style, 'large-field' ) ) ) {
            ...
            if ( is_int( stripos( $field['value']->style, 'medium-field' ) ) || is_int( stripos( $field['value']->style, 'date' ) ) ) {
                $widthcasses = "col-sm-5";
            } else {
                if ( is_int( stripos( $field['value']->style, 'small-field' ) ) || is_int( stripos( $field['value']->style, 'btn_switch' ) ) ) {
...
<script type="text/javascript">
    ...
    $('#test_connection').click(function () {
        if ($('#editSettings_default_pp_pro_status').attr('data-orgvalue') != '1') {
            error_alert(<?php abc_js_echo( $error_turn_extension_on ); ?>);
            return false;
        }
        $.ajax({
