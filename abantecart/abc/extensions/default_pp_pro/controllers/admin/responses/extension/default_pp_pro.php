    ...
    public function test()
    {
        ...
        $payment_data = [
            ...
            'USER'                           => html_entity_decode(
                                                        $cfg['default_pp_pro_username'],
                                                        ENT_QUOTES,
                                                        ABC::env('APP_CHARSET')
            ),
            'PWD'                            => html_entity_decode(
                                                        $cfg['default_pp_pro_password'],
                                                        ENT_QUOTES,
                                                        ABC::env('APP_CHARSET')
            ),
            'SIGNATURE'                      => html_entity_decode(
                                                        $cfg['default_pp_pro_signature'],
                                                        ENT_QUOTES,
                                                        ABC::env('APP_CHARSET')
            ),
            ...
            'CANCELURL'                      => $this->html->getSecureURL(
                                                        'extension/extensions/edit',
                                                        '&extension=default_pp_pro'
                                                ),
        ];
        ...
        if (H::has_value($this->request->get['order_id'])) {
            ...
            if (H::has_value($order_info['payment_method_data'])) {
                ...
                if ( $result['ACK'] != 'Success' ) {
                    ...
                    $this->model_extension_default_pp_pro->addOrderHistory(
                        [
            ...
            if ( $amount > 0 ) {
                ...
                if (H::has_value($order_info['payment_method_data'])) {
                    ...
                    if ( $this->request->get['refund_captured'] ) {
                        ...
                        $transaction_id = $payment_method_data['captured_transaction_id'];
                    } else {
                        $payment_amount = (float)$payment_method_data['AMT'];
                        $transaction_id = $payment_method_data['TRANSACTIONID'];
                    }
                    ...
                    if ( $amount > $payment_amount ) {
                        ...
                        if ( $result['ACK'] != 'Success' ) {
                            ...
                            // update order_totals
                            $this->loadModel( 'extension/default_pp_pro' );
                            $this->model_extension_default_pp_pro->processRefund( [
                                'order_id' => $this->request->get['order_id'],
                                'amount'   => $amount,
