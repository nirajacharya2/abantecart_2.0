...
class ControllerPagesExtensionBannerManager extends AController
{
    ...
    private function _getForm()
    {
        ...
        $groups = ['0' => $this->language->get( 'text_select' ), 'new' => $this->language->get( 'text_add_new_group' )];
        if ( $result ) {
    ...
    private function _validateForm()
    {
        ...
        if ( $this->request->post ) {
            ...
            if ( ! is_array( $this->request->post['banner_group_name'] )
                || ( ! trim( $this->request->post['banner_group_name'][1] ) && in_array( $this->request->post['banner_group_name'][0], ['0', 'new']) )
            ) {
    ...
    private function _getBlockForm()
    {
        ...
        if ( $custom_block_id ) {
            ...
            if ( $list ) {
                ...
                $assigned_banners = $this->model_extension_banner_manager->getBanners( ['subsql_filter' => 'b.banner_id IN ('.implode( ', ', $ids ).')']);
        ...
        if ( $this->data['block_wrapper'] && ! isset( $this->data['block_wrappers'][$this->data['block_wrapper']] ) ) {
            $this->data['block_wrappers'] = array_merge( [$this->data['block_wrapper'] => $this->data['block_wrapper']], $this->data['block_wrappers'] );
        }
