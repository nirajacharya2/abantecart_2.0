...
if($product['rating']){?>
	...
	if($('<?php echo $target;?> .abantecart_rating')){
		html = '<?php echo '<img src="' .ABC::env('AUTO_SERVER'). $this->templateResource('assets/images/stars_' . (int)$product['rating'] . '.png') . '" alt="' . $product['stars'] . '" />'?>';
...
if($product['quantity'] && !($product['track_stock'] && !$product['in_stock']) && !$product['call_to_order']){?>
	...
	if($('<?php echo $target;?> .abantecart_quantity')){
		html = '<span class="abantecart_quantity_text"><?php abc_js_echo($text_qty);?></span>&nbsp;<input type="text" size="3" class="abantecart_quantity_field" placeholder="<?php abc_js_echo($text_qty);?>" value="<?php echo $product['quantity']->value?>" id="product_quantity" name="<?php echo $product['quantity']->name?>"></div>';
...
if($product['button_addtocart']){?>
	...
	if($('<?php echo $target;?> .abantecart_addtocart')){
		...
		html ='';
		<?php }else{ ?>
		html ='<button <?php echo $product['button_addtocart']->attr; ?> title="<?php abc_js_echo($product['button_addtocart']->text); ?>" class="abantecart_button" id="<?php echo $product['button_addtocart']->id; ?>" type="button"><?php abc_js_echo($product['button_addtocart']->text); ?></button>';
	<?php } ?>
...
if($product['blurb']){?>
	if($('#<?php echo $target;?> .abantecart_blurb')){
		$('#<?php echo $target;?> .abantecart_blurb').html('<?php abc_js_echo($product['blurb']); ?>');
	}
<?php }
