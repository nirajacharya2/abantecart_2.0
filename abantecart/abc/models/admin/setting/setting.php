...
class ModelSettingSetting extends Model
{
    ...
    {
        $data = [];
        $query = $this->db->query("SELECT DISTINCT `group` FROM ".$this->db->table_name("settings")." ");
    ...
    public function editSetting($group, $data, $store_id = null)
    {
        ...
        if (H::has_value($data['config_url'])) {
            $url_protocol = preg_match("/^(https):\/\//", $data['config_url']) ? 'https' : 'http';
        }
        if (H::has_value($data['config_ssl_url'])) {
        ...
        if (H::has_value($data['config_url']) && substr($data['config_url'], -1) != '/') {
            $data['config_url'] .= '/';
        }
        if (H::has_value($data['config_ssl_url']) && substr($data['config_ssl_url'], -1) != '/') {
            $data['config_ssl_url'] .= '/';
        }
        //need to set ssl_mode setting to use it in AHtml class for building correct URLs
        if (!H::has_value($data['config_url']) XOR !H::has_value($data['config_ssl_url'])) {
            $saved_settings = $this->getSetting($group, $store_id);
            if (!H::has_value($data['config_url']) && H::has_value($data['config_ssl_url'])) {
        ...
        foreach ($data as $key => $value) {
            ...
            if($result->num_rows > 1){
                $sql = "DELETE FROM ".$this->db->table_name("settings")." 
                        WHERE `group` = '".$this->db->escape($group)."'
                                AND `key` = '".$this->db->escape($key)."'
                                AND `store_id` = '".$store_id."'";
                $this->db->query($sql);
                ...
                $setting_id = $result->row['setting_id'];
            }

            if(!$setting_id) {
                $sql = "INSERT INTO ".$this->db->table_name("settings")." 
