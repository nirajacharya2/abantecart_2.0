...
class ModelToolMigration extends Model
{
    ...
    public function isCartSupported($cart)
    {
        ...
        require_once($file);
        $name = "abc\models\admin\\".self::CLASS_PREFIX.ucfirst($cart);
    ...
    public function saveStepData($vars)
    {
        foreach ($vars as $var) {
            ...
            $this->session->data['migration'][$var] =
                !empty($this->request->post[$var]) ? $this->request->post[$var].$separator : '';
        }
    }
    ...
    protected function clearData()
    {
        ...
        if ($this->session->data['migration']['migrate_products']) {
            //categories
            $sql .= "DELETE FROM `".$this->db->table_name("categories")."`;\n";
            $sql .= "DELETE FROM `".$this->db->table_name("category_descriptions")."` 
                    WHERE language_id='".$languageId."';\n";
            $sql .= "DELETE FROM `".$this->db->table_name("categories_to_stores")."`;\n";
            $sql .= "DELETE FROM `".$this->db->table_name("url_aliases")."` 
                    WHERE query LIKE 'category_id=%';\n";
            $sql .= "DELETE FROM `".$this->db->table_name("resource_map")."` 
                    WHERE object_name = 'categories';\n";
            //products
            $sql .= "DELETE FROM `".$this->db->table_name("products")."`;\n";
            $sql .= "DELETE FROM `".$this->db->table_name("product_descriptions")."` 
                    WHERE language_id='".$languageId."';\n";
            $sql .= "DELETE FROM `".$this->db->table_name("products_to_categories")."`;\n";
            $sql .= "DELETE FROM `".$this->db->table_name("products_to_downloads")."`;\n";
            $sql .= "DELETE FROM `".$this->db->table_name("products_to_stores")."`;\n";
            $sql .= "DELETE FROM `".$this->db->table_name("product_options")."`;\n";
            $sql .= "DELETE FROM `".$this->db->table_name("product_option_descriptions")."`;\n";
            ...
            $sql .= "DELETE FROM `".$this->db->table_name("reviews")."`;\n";
            $sql .= "DELETE FROM `".$this->db->table_name("manufacturers")."`;\n";
            $sql .= "DELETE FROM `".$this->db->table_name("manufacturers_to_stores")."`;\n";
            $sql .= "DELETE FROM `".$this->db->table_name("url_aliases")."` 
        ...
        if ($this->session->data['migration']['migrate_customers']) {
            $sql .= "DELETE FROM `".$this->db->table_name("customers")."`;\n";
            $sql .= "DELETE FROM `".$this->db->table_name("addresses")."`;\n";
        }
    ...
    function clearCache()
    {
        $this->cache->flush('category');
        $this->cache->flush('category_description');
        $this->cache->flush('manufacturer');
        $this->cache->flush('product');
        $this->cache->flush('product_image');
        $this->cache->flush('product_option');
        $this->cache->flush('product_option_description');
        $this->cache->flush('product_option_value');
        $this->cache->flush('product_option_value_description');
        $this->cache->flush('product_to_category');
        $this->cache->flush('url_alias');
        $this->cache->flush('product_special');
        $this->cache->flush('product_discount');
    }
    ...
    protected function migrateCustomers()
    {
        ...
        foreach ($customers as $data) {
            ...
            $store_id = H::has_value($data['store_id'])
                ? (int)$data['store_id']
                : (int)$this->config->get('config_store_id');
            $date_added = H::has_value($data['date_added'])
                ? "'".$this->db->escape($data['date_added'])."'"
                : 'NOW()';
            $status = H::has_value($data['status']) ? $data['status'] : 1;
            $approved = H::has_value($data['approved']) ? $data['approved'] : 1;
            ...
            if (!$loginname && !$this->config->get('prevent_email_as_login') && $data['email']
                && !in_array($data['email'], $logins)) {
            ...
            foreach ($data['address'] as $address) {
                $sql = "INSERT INTO ".$this->db->table_name("addresses")."
                      SET customer_id = '".(int)$customer_id."',
                          firstname = '".$this->db->escape($address['firstname'])."',
                            ...
                            country_id = ".($address['country_iso_code2'] ? " COALESCE((SELECT country_id
                                                                                FROM "
                        .$this->db->table_name("countries")."
                                                                                WHERE iso_code_2='"
                        .$this->db->escape($address['country_iso_code2'])."'
                                                                                LIMIT 0,1),0)"
                        : "'0'").",
                            zone_id = ".($address['zone_iso_code2'] ? "COALESCE((SELECT zone_id
                                                                            FROM ".$this->db->table_name("zones")."
                                                                            WHERE code='"
                        .$this->db->escape($address['zone_iso_code2'])."'
                                                                            LIMIT 0,1),0)"
                        : "'0'");
            ...
            $result = $this->db->query("UPDATE ".$this->db->table_name("customers")."
    ...
    protected function migrateProducts()
    {
        ...
        foreach ($categories as $data) {
            ...
            $result = $this->db->query("INSERT INTO ".$this->db->table_name("categories")."
            ...
            $result = $this->db->query("INSERT INTO ".$this->db->table_name("category_descriptions")."
            ...
            $result = $this->db->query("INSERT INTO ".$this->db->table_name("categories_to_stores")."
        ...
        $query = $this->db->query("SELECT category_id, parent_id FROM ".$this->db->table_name("categories"));
        foreach ($query->rows as $result) {
            if (empty($category_id_map[$result['parent_id']])) {
                continue;
            }

            $result = $this->db->query("UPDATE ".$this->db->table_name("categories")."
...
// import manufacturers
        ...
        foreach ($manufacturers as $data) {
            ...
            $result = $this->db->query("INSERT INTO ".$this->db->table_name("manufacturers")."
            ...
            $result = $this->db->query("INSERT INTO ".$this->db->table_name("manufacturers_to_stores")."
        ...
        foreach ($products as $data) {
            ...
            $data['manufacturer_id'] =
                empty($manufacturer_id_map[$data['manufacturer_id']]) ? '' : $manufacturer_id_map[$data['manufacturer_id']];
            $date_added =
                H::has_value($data['date_added']) ? "'".$this->db->escape($data['date_added'])."'" : 'NOW()';
            $date_modified =
                H::has_value($data['date_modified']) ? "'".$this->db->escape($data['date_modified'])
                    ."'" : 'NOW()';

            $result = $this->db->query("INSERT INTO ".$this->db->table_name("products")."
            ...
            $result = $this->db->query("INSERT INTO ".$this->db->table_name("product_descriptions")."
            ...
            if ($this->config->get('enable_seo_url')) {
                if (!$data['seo_keyword']) {
                    $seo_key = H::SEOEncode($data['name'], 'product_id', $product_id);
                } else {
                    $seo_key = H::SEOEncode($data['seo_keyword'], 'product_id', $product_id);
                }
            ...
            $result = $this->db->query("INSERT INTO ".$this->db->table_name("products_to_stores")."
            ...
            if (isset($data['product_category'])) {
                foreach ($data['product_category'] as $category_id) {
                    ...
                    $result = $this->db->query("INSERT INTO ".$this->db->table_name("products_to_categories")."
            ...
            if ($data['reviews']) {
                ...
                foreach ($data['reviews'] as $review) {
                    ...
                    $sql = "INSERT INTO ".$this->db->table_name("reviews")."
        ...
        foreach ($options['product_options'] as $product_option) {
            //options
            $required = H::has_value($product_option['required']) ? $product_option['required'] : 0;
            $sql = "INSERT INTO ".$this->db->table_name("product_options")." (product_id, sort_order,status,element_type,required)
                    VALUES('".$product_id_map[$product_option['product_id']]."','".$product_option['sort_order']
                ."','1','".$this->db->escape($product_option['element_type'])."', '".(int)$required."');";
            ...
            $key = empty($product_option['product_option_id']) ? $product_option['product_id'].'_new_'
                .$product_option['products_text_attributes_id'] : $key;

            $product_option_id_map[$key] = $product_option_id;

            //option description
            $sql = "INSERT INTO ".$this->db->table_name("product_option_descriptions")." (product_option_id, language_id, product_id, name)
                    VALUES('".$product_option_id."', 1, '".$product_id_map[$product_option['product_id']]."','"
        ...
        foreach ($options['product_option_values'] as $product_option_value) {
            ...
            $sql = "INSERT INTO ".$this->db->table_name("product_option_values")." (product_id,
                                                                product_option_id,
                                                                price,
                                                                quantity,
                                                                weight,
            ...
            $sql = "INSERT INTO ".$this->db->table_name("product_option_value_descriptions")." (
    ...
    public function getCartList()
    {
        ...
        if ($files) {
            foreach ($files as $file) {
                ...
                if (strtolower($cart_name) == 'interface_migration') {
                    continue;
                }
        ...
        foreach ($data['image'] as $k => $source) {
            ...
            if ($src_exists) {
                ...
                if (($file = $this->downloadFile($source)) === false) {
                    $this->is_error = true;
                    if ($k == 'db') {
                        $this->addLog($objects[$object_txt_id]." ".$data['name']." File  ".$source
                ...
                if (!$this->is_error) {
                    ...
                    if (!$this->writeToFile($file, $target)) {
                        $this->is_error = true;
                        if ($k == 'db') {
                            $this->addLog("Cannot create ".$objects[$object_txt_id]." ".$data['name']." ( ".$source
                                ." )  file ".$target." in resource/image folder ");
                        }
                        continue;
                    }
                    $resource = [
