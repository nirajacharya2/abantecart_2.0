...
class ModelLocalisationLocation extends Model
{
    ...
    public function addLocationZone($location_id, $data)
    {
        ...
        $sql = "INSERT INTO ".$this->db->table_name("zones_to_locations")
    ...
    public function editLocation($location_id, $data)
    {
        ...
        if (!empty($update)) {
            $this->db->query("UPDATE ".$this->db->table_name("locations")." SET ".implode(',', $update)
    ...
    public function editLocationZone($zone_to_location_id, $data)
    {
        ...
        if (!empty($update)) {
            $this->db->query("UPDATE ".$this->db->table_name("zones_to_locations")." SET ".implode(',', $update)
    ...
    public function deleteLocation($location_id)
    {
        $this->db->query("DELETE FROM ".$this->db->table_name("locations")." WHERE location_id = '".(int)$location_id
            ."'");
        $this->db->query("DELETE FROM ".$this->db->table_name("zones_to_locations")." WHERE location_id = '"
            .(int)$location_id."'");
        $this->cache->flush('localization');
    }
    ...
    {
        $this->db->query("DELETE FROM ".$this->db->table_name("zones_to_locations")." WHERE zone_to_location_id = '"
            .(int)$zone_to_location_id."'");
        $this->cache->flush('localization');
    }
    ...
    {
        $query = $this->db->query("SELECT DISTINCT * FROM ".$this->db->table_name("locations")." WHERE location_id = '"
            .(int)$location_id."'");
    ...
    {
        $query = $this->db->query("SELECT * FROM ".$this->db->table_name("zones_to_locations")
            ." WHERE zone_to_location_id = '".(int)$zone_to_location_id."'");
    ...
    public function getZoneToLocations($data)
    {
        ...
        $sql = "SELECT zl.*, COALESCE( cd1.name,cd2.name) as country_name, COALESCE( zd1.name, zd2.name) as name
                ...
                LEFT JOIN ".$this->db->table_name("country_descriptions")
            ...
            ." cd2 ON (c.country_id = cd2.country_id AND cd2.language_id = '".(int)$default_language_id."')
                ...
                LEFT JOIN ".$this->db->table_name("zone_descriptions")
            ." zd1 ON (z.zone_id = zd1.zone_id AND zd1.language_id = '".(int)$language_id."')
                LEFT JOIN ".$this->db->table_name("zone_descriptions")
            ." zd2 ON (z.zone_id = zd2.zone_id AND zd2.language_id = '".(int)$default_language_id."') 
    ...
    {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM ".$this->db->table_name("zones_to_locations")
