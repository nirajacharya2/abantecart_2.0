...
class ModelLocalisationStockStatus extends Model
{
    public function addStockStatus($data)
    {
        ...
        $result =
            $this->db->query("SELECT MAX(stock_status_id) as max_id FROM ".$this->db->table_name("stock_statuses")." ");
    ...
    {
        $this->db->query("DELETE FROM ".$this->db->table_name("stock_statuses")." 
    ...
    public function getStockStatus($stock_status_id)
    {
        $query = $this->db->query("SELECT *
									FROM ".$this->db->table_name("stock_statuses")." 
									WHERE stock_status_id = '".(int)$stock_status_id."'
    ...
    public function getStockStatuses($data = [])
    {
        ...
        if ($data) {
            $sql = "SELECT * FROM ".$this->db->table_name("stock_statuses")." WHERE language_id = '".$language_id."'";
            ...
            $stock_status_data = $this->cache->get($cache_key);

            if ($stock_status_data === null) {
                $query = $this->db->query("SELECT stock_status_id, name
											FROM ".$this->db->table_name("stock_statuses")." 
											WHERE language_id = '".$language_id."'
											ORDER BY name");
                $stock_status_data = $query->rows;
                $this->cache->put($cache_key, $stock_status_data);
            }
    ...
    public function getStockStatusDescriptions($stock_status_id)
    {
        ...
        $query = $this->db->query("SELECT * FROM ".$this->db->table_name("stock_statuses")." WHERE stock_status_id = '"
            .(int)$stock_status_id."'");
    ...
    {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM ".$this->db->table_name("stock_statuses")
