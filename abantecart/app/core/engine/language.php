...
class ALanguage{
	...
	public function __construct($registry, $code = '', $section = 0){
		...
		if ($result !== false) {
			/**
			 * @var \abc\model\admin\ModelLocalisationLanguage $model
			 */
	...
	public function isMainBlock($block, $lang_id = ''){
		if (AHelperUtils::has_value($lang_id)) {
		...
		if (empty($lang_value) && $this->registry->get('config')->get('warn_lang_text_missing')) {
			...
			if (!$silent) {
				$this->registry->get('messages')->saveWarning('Language definition "' . $key . '" is missing for "' . $this->available_languages[$this->code]['name'] . '"',
						'AbanteCart engine cannot find value of language definition with key "' . $key . '" in ' . $caller_file . ' line ' . $caller_file_line . ($rt ? ' (rt=' . $rt . ')' : '') . '.  Please add it in #admin#rt=localisation/language_definitions or run language translate process in #admin#rt=localisation/language');
			}
		}
		return $lang_value;
	}
