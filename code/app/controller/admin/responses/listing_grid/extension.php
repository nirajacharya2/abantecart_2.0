...
class ControllerResponsesListingGridExtension extends AController{
	...
	public function main(){
		...
		foreach ($rows as $row){
			...
			if ($row['remote_install']){
			...
			} elseif (!file_exists(DIR_APP_EXT . $extension . '/main.php') || !file_exists(DIR_APP_EXT . $extension . '/config.xml')){
				$response->userdata->classes[$id] = 'warning disable-edit disable-install disable-uninstall disable-remote-install';
				$icon = '<img src="' . RDIR_TEMPLATE . 'image/default_extension.png' . '" alt="" border="0" />';
				...
				$icon_ext_dir = DIR_APP_EXT . $extension . '/image/icon.png';
	...
	public function license(){
		...
		if ($result){
			...
			if (file_exists(DIR_APP_EXT . $this->request->get['extension'] . "/license.txt")){
				$this->data['license_text'] = file_get_contents(DIR_APP_EXT . $this->request->get['extension'] . "/license.txt");
