...
class ControllerPagesToolPackageInstaller extends AController {
	...
	*/
	...
	private function _installExtension($extension_id = '', $confirmed = false, $agree = 0) {
		...
		if ($already_installed || file_exists(DIR_EXT_APP . $extension_id)) {
			if(!is_writable(DIR_EXT_APP . $extension_id)){
				$this->session->data['error'] = $this->language->get('error_move_backup').DIR_EXT_APP . $extension_id;
				$this->redirect($this->_get_begin_href());
			}else{
		...
		if ($package_info['ftp']) {
			...
			$result = rename($temp_dirname . $package_dirname . "/code/extensions/" . $extension_id, DIR_EXT_APP.$extension_id);
			//this method requires permission set to be set
			$pmanager->chmod_R(DIR_EXT_APP.$extension_id ,0777, 0777);
		}
		...
		if ($result) {
			...
			if ($package_info['ftp']) {
				$this->session->data['error'] = $this->language->get('error_move_ftp') . DIR_EXT_APP . $extension_id.'<br><br>'.$pmanager->error;
				$this->redirect($this->html->getSecureURL('tool/package_installer/agreement'));
			} else {
				$this->session->data['error'] = $this->language->get('error_move') . DIR_EXT_APP . $extension_id.'<br><br>'.$pmanager->error;
				$this->_removeTempFiles('dir');
				$this->redirect($this->_get_begin_href());
			}
		}
